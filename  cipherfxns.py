...
def transposition():
    #ask for user input for message and key
    plain = input('Enter the message: ')
    key = input('Enter the key (column permutation of length 7): ')
    ...
    #loop through key
    for x in range(1,8):
        #loop through text to add the columns in order 1-7
        for y in range(0,len(plain)):
            if y % 7 == key.index(str(x)):
                cipher += plain[y]
    ...
    #decipher
    #loop through ciphertext
    #go through key, go through each set of column
    #get first from first in key, first from second in key, etc.
    #all the way through the set of column
    for x in range(len(cipher)//7):
        for y in range(0, len(key)):
            decipher += cipher[4*(int(key[y])-1)+x]
    print('plaintext:', decipher)